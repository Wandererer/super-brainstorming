...
- **research_first 모드 (검색 발굴 — "research 후 brainstorming")**: 증거 마이너 6종 + 합성 렌즈 3종
- 공통: 하이브리드/선행기술 대조(Phase 4.5)/red-team/심사 프롬프트
...
## The 6 Miners (evidence id blocks: ev_1xx ~ ev_6xx)
...
### M6. `solution_saturation_miner` — 기존 해법 채굴 (ev_6xx) *(공급측)*
```
MINER: Solution Saturation Miner.
The other miners dig DEMAND (pains, gaps). You dig SUPPLY: what already exists.
Search "best/top {category} tools {YEAR}", App Store/Play charts, Product Hunt,
GitHub (stars/activity for OSS), "{use case} app" — enumerate products already
serving each niche in the frame's space. For each product emit one card:
- "claim": 제품명 + 무엇을 해결 + traction 신호 (평점·리뷰수·stars 등 숫자로)
- "pricing": "free" | "freemium" | "paid" | "oss" | "unknown"  ← 필수 필드
type: existing_solution.
Honesty over optimism — finding a great free incumbent is a SUCCESS of this
pass, not a failure. You are the reason me-too ideas die early.
(competitor_gap_miner와 역할 분담: 그쪽은 강자의 빈틈/기회를, 너는 니치별
포화도를 본다.)
```
...
```python
# Phase 3a — miners (기본 6종), throttled 2-3 concurrent (Rate-Limit Guard)
...
# ... solution_saturation_miner(ev_6xx)까지 배치 순차 실행
...
# Phase 4.5 — prior-art agent 1개 (양 모드 공통, 클러스터 단위 검색)
# existing_solution 카드(ev_7xx)를 evidence ledger에 append + saturation 블록을
# idea_id로 매칭해 ledger에 병합 (red-team 병합과 같은 방식)
Task(subagent_type="general-purpose", prompt=prior_art_prompt(clusters, ev_block="7xx"))
```
...
## Phase 4.5: Prior-Art Agent (선행기술 대조 — 양 모드 공통)

```
You are the prior-art agent. Divergence is over. Today's date: {TODAY}.
FRAME: {HMW + anti_goals}

Below are the idea CLUSTERS (grouped by theme — search per cluster, NOT per
card; that is the cost contract):
{IDEA_CLUSTERS — cluster name + member idea_ids/titles/one_liners}

For EACH cluster:
1. Run 1-2 targeted searches: "best {cluster category} tools {YEAR}",
   "{core mechanism} app/product", Product Hunt, GitHub. Append the year.
2. Register every existing product you find as an existing_solution evidence
   card (JSONL, id block ev_7xx, source_url 필수, "pricing" 필수 —
   free|freemium|paid|oss|unknown). No URL → no card. 경쟁사도 지어내지 않는다.

Then for EVERY idea, emit one saturation line:
{"idea_id": "...", "saturation": {"checked": true, "score": 0-5, "competitor_ids": ["ev_7xx", ...], "wedge": null | "한 문장 — 기존 해법 대비 무엇이 다르고 왜 그들이 구조적으로 못 따라오나", "note": "score 0이면 '검색함, 없음'"}}

score anchors (scoring_rubric.md SSOT):
0 = 검색했으나 같은 문제를 푸는 제품 없음 · 1 = 인접 제품이 부분 커버 ·
2 = 1-2개 존재하나 미성숙/불만 많음 · 3 = 성숙한 제품 여럿 (유료 중심) ·
4 = 성숙한 제품 다수 + 무료/OSS 존재 · 5 = 상품화 완료 — 무료 포함 다수가 잘함

Rules:
- You do not kill ideas and you do not score novelty. You map the supply side.
- wedge는 카드의 mechanism에서 읽어낼 수 있을 때만 쓴다 — 없으면 null로 두고
  게이트가 강제하게 한다. "더 잘 만들겠다"는 wedge가 아니다.
- 게이트(validate_ideas.py)가 이 데이터로 novelty를 상한 처리한다: 정직이 전부다.
```
...
- ME-TOO CHECK (near-fatal): each card carries a saturation block with
  competitor evidence (Phase 4.5). If a mature FREE/OSS product already does
  this well (saturation 4-5, or 3 with a free competitor) and the card's wedge
  is empty or hollow ("we'll just do it better"), that IS a fatal_flaw —
  state it as "성숙한 무료 해법 존재({competitor}), wedge 부재". A concrete
  structural wedge (why incumbents CANNOT follow) saves the card → judges.
...
CANDIDATES (with saturation blocks + competitor evidence attached):
{CANDIDATE_CARDS_WITH_SATURATION}
```
...
- Score EVERY axis for EVERY candidate. No omissions, no ties-by-default —
  ...
  flagged by the gate.
- NOVELTY IS TOOL-GROUNDED: each candidate carries a saturation block and
  competitor evidence from the prior-art pass (Phase 4.5). Score novelty
  ONLY against that attached list — never from memory or vibes. Empty
  competitor list + note "검색함, 없음" is positive evidence of novelty.
  (The gate caps novelty by code when the niche is saturated — score honestly.)
...
CANDIDATES (with red-team objections + saturation blocks attached):
{CANDIDATE_CARDS_WITH_RED_TEAM_AND_SATURATION}
```
